...
def is_port_in_use(host, port):
    """Return True if the (host, port) is already in use."""
...
def find_free_port(host, start_port=8000, max_port=8100):
    """Find a free TCP port on `host` between `start_port` and `max_port`.

    Returns port number or None if none available.
    """
...
def main():
    ...
    # Check whether the requested ML backend port is free; if not, find an alternative.
    ml_port = BACKEND_PORT
    if is_port_in_use(BACKEND_HOST, BACKEND_PORT):
        print(f"⚠ Port {BACKEND_PORT} already in use — searching for an available port...")
        alt = find_free_port(BACKEND_HOST, BACKEND_PORT + 1, BACKEND_PORT + 100)
        if alt:
            print(f"✓ Found free port {alt}; using it for ML backend.")
            ml_port = alt
        else:
            print("✗ No free ports found near requested port — ML backend will not be started.")

    # Start backend only if we have a valid port
    if ml_port is not None:
        try:
    ...
    if ml_proc and ml_proc.poll() is None:
        print(f"ML API:  http://{BACKEND_HOST}:{ml_port}")
    else:
